...
def srd_router_with_mocks(
    ...
    with patch("api_backend.routers.srd.DatabaseService") as mock_db_class:
        with patch("api_backend.routers.srd.S3Client") as mock_s3_class:
            mock_db_class.return_value = mock_db_service

            def s3_client_side_effect(bucket_name):
                if "documents" in bucket_name:
                    return mock_s3_client_docs
                elif "vector" in bucket_name:
                    return mock_s3_client_vectors
                return mock_s3_client_docs

            mock_s3_class.side_effect = s3_client_side_effect
...
class TestGetPresignedUploadUrl:
    ...
    def test_get_presigned_upload_url_with_default_content_type(
        ...
        srd_id = "test_srd"
        request = PresignedUrlRequest(
            file_name="test_document.pdf", content_type=None
        )
    ...
    def test_get_presigned_upload_url_db_error(
        ...
        """Test presigned URL generation with database error."""
        # Arrange
        srd_id = "test_srd"
        request = PresignedUrlRequest(
            file_name="test_document.pdf", content_type="application/pdf"
    ...
    def test_get_presigned_upload_url_request_parsing_error(
        self, srd_router_with_mocks, mock_user
    ):
        """Test presigned URL generation with request parsing error."""
        # Arrange
        srd_id = "test_srd"
        # Create a mock request that will raise an exception when accessed
        mock_request = MagicMock()
        # Make file_name property raise an exception when accessed
        type(mock_request).file_name = PropertyMock(
            side_effect=Exception("Parsing error")
        )
        mock_request.content_type = "application/pdf"
        mock_request.model_dump_json.return_value = "mock_json"

        # Act
        response = srd_router_with_mocks.get_presigned_upload_url(
            srd_id=srd_id, current_user=mock_user, request=mock_request
        )

        # Assert
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "error" in response.body.decode()
        assert "Error processing request data" in response.body.decode()
...
class TestRouterIntegration:
    ...
    def test_router_endpoints_exist(self, srd_router_with_mocks):
        """Test that all expected endpoints exist in the router."""
